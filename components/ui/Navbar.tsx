  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-white/80 backdrop-blur-md border-b border-stone-200">
      <nav className="mx-auto max-w-[1600px] flex h-16 items-center justify-between px-6 md:px-12">
        <Link href={`/${lang}`} className="text-xl font-medium text-stone-800 tracking-tight">
          Xedric Crochet
        </Link>
        <div className="flex items-center gap-4">
          ...
          >
            <span className={lang === 'id' ? 'text-stone-900 font-bold' : ''}>ID</span>
            {' | '}
            <span className={lang === 'en' ? 'text-stone-900 font-bold' : ''}>EN</span>
          </Link>
        </div>
      </nav>
    </header>
